...
def correct_emotion(text, bert_emotion):
    """Override BERT's emotion if the text is clearly negative but BERT said positive."""
    if bert_emotion in ("joy", "love", "surprise"):
        text_lower = text.lower()
        # Check individual negative words
        if set(text_lower.split()) & NEGATIVE_INDICATORS:
            return "sadness"
        # Check negation patterns (catches "not that good", "meh", etc.)
