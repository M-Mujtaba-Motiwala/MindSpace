...
from model.predict import predict_intent, load_classifier
...
# ── HELPERS ───────────────────────────────────────────────────────────────────
def pick(options):
    avail = [o for o in options if o != st.session_state.last_shortcut]
...
def reset_chat_state():
    for k in ["messages","intent_history","mentioned_topics"]:
        st.session_state[k] = []
...
def expert_system_rule(text, row):
    ...
    if str(row["escalation_required"]).strip().lower() in ["true","yes"]:
...
def get_bot_response(user_text):
    ...
    intent, emotion = predict_intent(user_text)
    ...
    if row is not None:
        ...
        core = clean_response(row["bot_response"])
